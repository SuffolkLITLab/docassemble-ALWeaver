...
def fill_in_field_attributes(new_field, pdf_field_tuple):
    ...
    new_field.transformed_variable = map_names(
        pdf_field_tuple[0]
    )  # TODO: wrap in varname
    ...
    if new_field.variable.endswith("_date"):
        ...
        new_field.variable_name_guess = "Date of " + new_field.variable[:-5].replace(
            "_", " "
        )
        ...
        new_field.variable_name_guess = (
            new_field.variable[:-3].replace("_", " ").capitalize()
            if new_field.variable.endswith("_no")
            else new_field.variable[:-4].replace("_", " ").capitalize()
        )
        ...
        for x in self.elements:
            if x.type == "md":
                output_list.append(
                    ...
                    + x.markdown_filename
                    + "`]("
                    + docassemble.base.functions.url_of(
                        "playgroundfiles",
                        section="template",
                        file=x.markdown_filename,
                        project=project,
                    )
                    + ")"
                )
                ...
                output_list.append(
                    ...
                    + "`]("
                    + docassemble.base.functions.url_of(
                        "playgroundfiles", section="template", project=project
                    )
                    + ")"
                )
            elif x.type == "docx":
    ...
    def source(self, follow_additional_fields=True):
        ...
        if self.type == "question":
            ...
            if hasattr(self, "has_mandatory_field") and not self.has_mandatory_field:
                content += (
                    "continue button field: " + varname(self.question_text) + "\n"
                )
            elif hasattr(self, "continue_button_field"):
                content += (
                    "continue button field: "
                    + varname(self.continue_button_field)
                    + "\n"
                )
            ...
            if self.field_list[0].field_type == "end_attachment":
                ...
                ):  # this tells us its the ending screen
                    # content += "buttons:\n  - Exit: exit\n  - Restart: restart\n" # we don't want people to erase their session
                    content += (
                        "attachment code: " + self.attachment_variable_name + "\n"
                    )
                ...
                for attachment in self.attachments:  # We will only have ONE attachment
                    ...
                    content += "attachment:\n"
                    content += (
                        "    variable name: " + self.attachment_variable_name + "\n"
                    )
                    ...
                    if attachment.type == "md":
                        ...
                        content += (
                            "    pdf template file: "
                            + oneline(attachment.pdf_filename)
                            + "\n"
                        )
                        ...
                        for field in attachment.fields:
                            ...
                            if (
                                hasattr(field, "field_data_type")
                                and field.field_data_type == "date"
                            ):
                                content += (
                                    '      - "'
                                    + field.variable
                                    + '": ${ '
                                    + varname(field.variable).format()
                                    + " }\n"
                                )
                            elif (
                                hasattr(field, "field_data_type")
                                and field.field_data_type == "currency"
                            ):
                                content += (
                                    '      - "'
                                    + field.variable
                                    + '": ${ currency('
                                    + varname(field.variable)
                                    + " ) }\n"
                                )
                            else:
                                ...
                                content += (
                                    ...
                                    + field.variable
                                    + '": ${ '
                                    + map_names(varname(field.variable))
                                    + " }\n"
                                )
                    elif attachment.type == "docx":
                        content += (
                            "    docx template file: "
                            + oneline(attachment.docx_filename)
                            + "\n"
                        )
            ...
            content += (
                '  mark_unfilled_fields_empty(interview_metadata["'
                + self.interview_label
                + '"])\n'
            )
            content += (
                "  "
                + self.interview_label
                + "_preview_question # Pre-canned preview screen\n"
            )
            ...
            content += (
                "    interview_metadata.initializeObject('"
                + self.interview_label
                + "')\n"
            )
            ...
            content += "    ],\n"
            content += (
                "    'preferred court': '" + oneline(self.preferred_court) + "',\n"
            )
            content += "    'categories': [" + "\n"
            for category in self.categories.true_values():
                content += "      '" + oneline(category) + "',\n"
...
class PlaygroundSection(object):
    def __init__(self, section="", project="default"):
        ...
        self.user_id = docassemble.base.functions.this_thread.current_info["user"][
            "theid"
        ]
    ...
    def _update_file_list(self):
        ...
        self.file_list = sorted(
            [
                ...
                if f != ".placeholder"
                and os.path.isfile(os.path.join(the_directory, f))
            ]
        )
    ...
    def reduced_file_list(self):
        ...
        out_list = [
            ...
            if os.path.splitext(f)[1].lower() in [".md", ".pdf", ".docx"]
    ...
    def is_fillable_docx(self, filename):
        ...
        for variable in re.findall(
            r"{%[a-z]* for [A-Za-z\_][A-Za-z0-9\_]* in *([^\} ]+) *%}", result
        ):
            fields.add(docx_variable_fix(variable))
        if len(fields):
            return True
    ...
    def convert_file_to_md(self, filename, convert_variables=True):
        ...
        if convert_variables:
            with open(temp_file.name, "rU", encoding="utf-8") as fp:
                self.write_file(
                    out_filename,
                    replace_square_brackets.sub(fix_variable_name, fp.read()),
                )
        else:
...
class Playground(PlaygroundSection):
    ...
    def get_package_as_zip(self, pkgname):
        ...
        author_info["author name"] = (
            self.current_info["user"]["firstname"]
            + " "
            + self.current_info["user"]["lastname"]
        )
        author_info["author email"] = self.current_info["user"]["email"]
        author_info["author name and email"] = (
            author_info["author name"] + ", " + author_info["author email"]
        )
    ...
    def variables_from(self, content):
        the_directory = directory_for(self.get_area(), self.project)
        interview_source = docassemble.base.parse.InterviewSourceString(
            content=content,
            directory=the_directory,
            path="docassemble.playground"
            + str(self.user_id)
            + project_name(self.project)
            + ":_temp.yml",
        ...
        temp_current_info["yaml_filename"] = (
        ...
        interview_status = docassemble.base.parse.InterviewStatus(
            current_info=temp_current_info
        )
        ...
        avail_modules = set(
            [
                re.sub(r".py$", "", f)
                for f in os.listdir(the_directory)
                if os.path.isfile(os.path.join(the_directory, f))
            ]
        )
        ...
        for val in user_dict:
            if type(user_dict[val]) is types.FunctionType:
                ...
                type(user_dict[val]) is TypeType
                or type(user_dict[val]) is types.ClassType
            ):
                classes.add(val)
        ...
        names_used = names_used.difference(
            functions | classes | modules | avail_modules
        )
        ...
        return dict(
            ...
            all_names=all_names,
            all_names_reduced=all_names_reduced,
        )
...
reserved_prefixes = (
    r"^(user"  # deprecated, but still supported
...
def map_names(label):
  ...
  that will be the value for the attachment field."""
    ...
    suffix_as_attribute = turn_any_suffix_into_an_attribute(
        reserved_suffixes_map, suffix
    )
