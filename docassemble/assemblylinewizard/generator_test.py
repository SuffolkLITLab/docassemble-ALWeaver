...
class TestMapNames(unittest.TestCase):
    ...
    def test_single_individuals(self):
      try:
        result = self.assertEqual( 'str(plaintiffs[0])', map_names('plaintiff') )
        log( result, 'console' )
        return result
      except AssertionError as error:
        log( str(error), 'console' )
    ...
    def test_multiple_appearances(self):
      try:
        result = self.assertEqual( 'str(plaintiffs[0])', map_names('plaintiff__2') )
        log( result, 'console' )
        return result
      except AssertionError as error:
        log( str(error), 'console' )
    ...
    def test_multiple_individuals(self):
      try:
        result = self.assertEqual( 'str(plaintiffs[2-1])', map_names('plaintiff2') )
        log( result, 'console' )
        return result
      except AssertionError as error:
        log( str(error), 'console' )
    ...
    def mapped_tests(self):
      ...
      for scenario_input in scenarios:
        ...
        try:
          ...
          log( result, 'console' )
          #results.append({ "test": scenario_input, "result": result })
          passed.append(scenario_input + "[BR]")

        # The error should show us what specifically didn't match up
        except AssertionError as error:
          log( error, 'console' )
          #results.append({ "test": scenario_input, "result": error })
