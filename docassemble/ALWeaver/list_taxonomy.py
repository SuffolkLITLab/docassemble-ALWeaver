...
def get_LIST_codes(file_path: str, custom_order: Optional[List[str]] = None) -> List[Optional[Dict[str, str]]]:
    """
    ...
    def get_order(prefix: str) -> int:
        if custom_order is None:
            return 999999999999
        return custom_order.index(prefix) if prefix in custom_order else len(custom_order)
    ...
    df['Suffix'] = df['Code'].str.slice(2,)

    # Create a group mapping dictionary
    group_dict = df.loc[df['Suffix'] == '-00-00-00-00', ['Prefix', 'Title']].set_index('Prefix')['Title'].to_dict()
    ...
    def create_dict(row: pd.Series) -> Optional[Dict[str, str]]:
        if row['Suffix'] != '-00-00-00-00':
            return {"label": row['Title'], "value": row['Code'], "group": group_dict.get(row['Prefix'], "MISC")}
        else:
