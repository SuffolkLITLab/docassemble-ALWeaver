...
class TestEditorOutlineHeader(unittest.TestCase):
    def test_outline_header_switches_by_view(self):
        ...
        editor_html = (base / "data" / "templates" / "editor.html").read_text(
            encoding="utf-8"
        )
        editor_css = (base / "data" / "static" / "editor.css").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn(
            "return isInterviewView() && !state.searchQuery.trim();", editor_js
        )
        self.assertIn("function getBlockDisplayType(block)", editor_js)
        self.assertIn(
            "function buildFullOutlineOrderFromVisibleIds(visibleIds)", editor_js
        )
        ...
        self.assertIn(
            "enableAction = block.type === 'commented' ? 'enable' : 'comment'",
            editor_js,
        )
        ...
        self.assertIn("renderReviewBlock(block)", editor_js)
        self.assertIn("blockType === 'question' || blockType === 'review'", editor_js)
        self.assertIn('data-jump="reviews"', editor_html)
        self.assertIn('data-insert="review"', editor_html)
        self.assertIn("editor-outline-type-review", editor_css)
        self.assertIn("/api/draft-review-screen", editor_js)
        self.assertIn('id="review-new-field" data-symbol-role="variable"', editor_js)
        self.assertIn("role === 'variable'", editor_js)
        self.assertIn(
            "return all.map(function (name) { return { name: name, group: 'variables' }; });",
            editor_js,
        )
        self.assertIn(
            'data-field-prop="variable" data-symbol-role="variable"', editor_js
        )
        self.assertIn("function loadAvailableSymbols(forceRefresh)", editor_js)
        self.assertIn("loadAvailableSymbols(true)", editor_js)
        self.assertIn("refreshActiveSymbolPickers()", editor_js)
        self.assertIn(
            "native scrolling and scrollbar dragging inside the suggestion list",
            editor_js,
        )
        self.assertNotIn(
            "target.closest('#editor-symbol-typeahead') || target.id === 'editor-symbol-typeahead'",
            editor_js,
        )
        self.assertIn("renderMarkdownToolbar('review-item-button-'", editor_js)
        self.assertNotIn("Double-click to edit", editor_js)
        self.assertNotIn("custom YAML", editor_js)
