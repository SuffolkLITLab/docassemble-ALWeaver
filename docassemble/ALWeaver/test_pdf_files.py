...
class test_pdfs(unittest.TestCase):
    def test_validate_ignore_push_button(self):
        ...
        da_pdf = MockDAStaticFile(full_path=str(push_button_pdf), extension='pdf', mimetype='application/pdf')
        fields = DAFieldList()
        fields.add_fields_from_file(da_pdf)
        fields.gathered = True
        bad_fields = get_variable_name_warnings(fields)
        self.assertEqual(len(bad_fields), 0, f"Bad fields in test_push_button.pdf: {bad_fields}")
